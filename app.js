...
function details(){
 ...
 <div class="form"><input id="bn" placeholder="Full name"><input id="bp" placeholder="Mobile number"><input id="be" placeholder="Email (optional)"><textarea id="bnotes" placeholder="Notes — you can mention a gallery style you like"></textarea></div>
...
function renderDiary(){
 ...
 const cards=laid.map((item,i)=>{
   ...
   return `<button class="diary-appt diary-tone-${i%4}" style="top:${Math.max(0,(s-start)*ppm)}px;height:${Math.max(58,d*ppm)}px;left:${left}px;width:${colWidth}px" onclick="openDiaryBooking('${x.id}')">
      ...
      <small>${x.deposit?`£${x.deposit} deposit required`:"No deposit"}</small>
   </button>`;
 }).join("");
 ...
 el.innerHTML=`<div class="visual-diary">
   <div class="diary-toolbar">
     <button onclick="diaryMove(-1)">‹</button>
     <div><small>APPOINTMENTS</small><h3>${nice(date)}</h3></div>
     <button onclick="diaryMove(1)">›</button>
   </div>
   <div class="diary-actions"><button class="today-btn" onclick="diaryToday()">Today</button><span>Swipe / scroll sideways to view the diary →</span></div>
...
function galleryAddSave(){
 ...
 resizeGalleryImage(file,1200,.82).then(src=>{
   ...
   try{save()}catch(e){S.gallery.shift();return toast("Storage is full — delete an uploaded photo first")}
