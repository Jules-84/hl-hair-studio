...
function openCustomer(encodedPhone){
  ...
  const c=rows[0],
  up=rows.filter(b=>b.status!=="cancelled"&&b.date>=today()),
  past=rows.filter(b=>b.date<today()||["cancelled","no_show","completed"].includes(b.status));

  const card=b=>{
    const updateType=b.customerUpdateType==="rescheduled"?"Rescheduled":"Notes changed";

    const update=b.customerUpdate
      ? `<div style="margin-top:10px;padding:8px 10px;background:#fae2c9;border-radius:8px;font-size:12px;font-weight:700;color:#9a6234">
          ● CUSTOMER UPDATE · ${updateType}
        </div>`
      : "";
    ...
    const seen=b.customerUpdate
      ? `<button type="button" class="text-back" style="margin-top:8px" onclick="event.stopPropagation();acknowledgeCustomerUpdate('${b.id}')">Mark update as seen</button>`
      : "";

    return `<div class="customer-booking-detail">
      <div>
        <b>${esc(b.serviceName)}</b><br>
        <span>${nice(b.date)} at ${b.time}</span><br>
        <small>${b.status==="cancelled"?"Cancelled":b.status==="completed"?"Completed":b.status==="no_show"?"No-show":"Booked"}${b.bookingRef?` · Ref ${esc(b.bookingRef)}`:""}</small>
        ${update}
        ${notes}
      </div>
      <div style="text-align:right">
        <b>${b.price===0?"Free":"£"+b.price}</b>
        ${b.deposit?`<br><small>Deposit ${b.depositPaid?"paid":"due"}</small>`:""}
        <br><button type="button" class="text-back" style="margin-top:8px" onclick="event.stopPropagation();openAdminBookingEditor('${b.id}')">Edit</button>
        ${seen}
      </div>
    </div>`;
  };

  modal(`<div class="customer-profile">
    <span class="admin-kicker">CUSTOMER</span>
    <h2>${esc(c.name)}</h2>
    <div class="customer-contact">
      <b>Mobile</b><span>${esc(c.phone||"—")}</span>
      <b>Email</b><span>${esc(c.email||"—")}</span>
    </div>
    <h3>Upcoming appointments</h3>
    ${up.length?up.map(card).join(""):"<p>No upcoming appointments.</p>"}
    <h3>Booking history</h3>
    ${past.length?past.map(card).join(""):"<p>No previous appointments.</p>"}
    <div style="margin-top:24px">
      <button class="danger-btn" onclick="deleteCustomer('${encodeURIComponent(phone)}')">Delete customer</button>
      <p class="muted" style="margin-top:8px">Removes this person from your Customers list. Their appointment history is kept.</p>
    </div>
  </div>`);
}
