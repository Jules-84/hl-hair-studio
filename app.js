...
function openAddService(){
  $("#bookbody").innerHTML=`<div class="bookcard"><button class="text-back" onclick="dateStep()">← Back</button><h2>Add another service</h2><p class="category-help">Choose another service to add to this appointment.</p>${categoryMarkup("previewExtraService")}</div>`;
}
function previewExtraService(id){
  const extra=S.services.find(x=>x.id===id);
  if(!extra)return;
  if(extra.id===W.service?.id)return toast("That service is already selected");
  W.services=[W.service,extra];
  $("#bookbody").innerHTML=`<div class="bookcard"><button class="text-back" onclick="openAddService()">← Back</button><h2>Your services</h2><div class="summary"><div><b>${esc(W.service.name)}</b><br><small>${durationText(W.service.duration)}</small></div><div style="margin-top:12px"><b>${esc(extra.name)}</b><br><small>${durationText(extra.duration)}</small></div></div><p class="category-help" style="margin-top:16px">Great — the second service is being selected correctly. We’ll connect the combined appointment time in the next step.</p><button type="button" class="primary full" onclick="W.services=[W.service];dateStep()">Remove second service</button></div>`;
}
