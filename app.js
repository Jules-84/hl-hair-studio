...
function renderDiary(){
 ...
 const start=6*60,end=24*60,ppm=isPhone?1.05:.64,height=(end-start)*ppm;
 let hours="";
 for(let h=6;h<=24;h++){
 ...
 el.innerHTML=`<style>
 .diary-appt.deposit-due{background:#f3cccc!important;border-color:#df9999!important}
 .diary-appt.deposit-paid{background:#d8ead8!important;border-color:#a9c9a9!important}
 .diary-appt.customer-update{background:#f8dfc5!important;border-color:#dfb987!important}
 .diary-appt.status-completed{background:#e6d9ef!important;border-color:#c5add7!important}
 .diary-appt.status-closed{background:#e5e1df!important;border-color:#c8c0bc!important}
 .diary-toolbar .diary-colour-key span{display:inline-flex;align-items:center;gap:5px;white-space:nowrap}
 @media(max-width:900px){.diary-toolbar .diary-colour-key{font-size:10px!important;gap:7px!important}}
 </style><div class="visual-diary ${isPhone?"phone-timeline":""}">
   <div class="diary-toolbar">
     <button aria-label="Previous day" onclick="diaryMove(-1)">\u2039</button>
     <div class="diary-colour-key" style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;font-size:12px">
       <span><i style="display:inline-block;width:11px;height:11px;border-radius:50%;background:#f3cccc;border:1px solid #df9999"></i> Deposit due</span>
       <span><i style="display:inline-block;width:11px;height:11px;border-radius:50%;background:#d8ead8;border:1px solid #a9c9a9"></i> Deposit paid</span>
       <span><i style="display:inline-block;width:11px;height:11px;border-radius:50%;background:#f8dfc5;border:1px solid #dfb987"></i> Customer update</span>
       <span><i style="display:inline-block;width:11px;height:11px;border-radius:50%;background:#e6d9ef;border:1px solid #c5add7"></i> Completed</span>
       <span><i style="display:inline-block;width:11px;height:11px;border-radius:50%;background:#e5e1df;border:1px solid #c8c0bc"></i> Cancelled / no-show</span>
     </div>
...
function openDiaryBooking(id){
 ...
 modal(`<h2>${esc(b.name)}</h2>${b.customerUpdate?`<div style="background:#f8dfc5;border:1px solid #dfb987;border-radius:12px;padding:12px;margin-bottom:12px"><b>New customer update</b><br><small>${b.customerUpdateType==="rescheduled"?"Customer rescheduled this appointment.":"Customer added or changed their notes."}</small></div>`:""}<div class="summary"><b>${esc(b.serviceName||b.service||"Appointment")}</b>${b.pinCurls?"<br>+ Pin Curls":""}<br>${nice(b.date)} at ${b.time}<br><b>Total: ${Number(b.price||0)===0?"Free":"£"+Number(b.price||0)}</b><br>${esc(b.phone||"")}${b.email?`<br>${esc(b.email)}`:""}${b.notes?`<br><br>Notes: ${esc(b.notes)}`:""}</div><button class="primary full" style="margin:14px 0" onclick="openAdminBookingEditor('${b.id}')">Edit appointment</button>${b.customerUpdate?`<button class="full" style="margin:0 0 14px" onclick="acknowledgeCustomerUpdate('${b.id}')">Mark customer update as seen</button>`:""}${statusHtml}${depositHtml}`);
}
